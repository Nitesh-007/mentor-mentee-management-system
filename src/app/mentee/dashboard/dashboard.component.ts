...
export class DashboardComponent implements OnInit {
  ...
  constructor(private menteeData: MenteeServiceService) {
  ...
  getTaskData() {
    ...
    console.log(utype)
    this.menteeData.menteeTask(utype).subscribe((data: any) => {
      this.taskSummary = data;
      console.warn(this.taskSummary);
    })
  }
  ...
  weeklyData() {
    ...
    return (this.menteeData.weekly(utype).subscribe((response: any) => {
      ...
      response.forEach((element: any) => {
        finalData.push({ name: element.dataType, y: element.mark })
      });
      this.isValue = 1;
  ...
  monthlyData() {
    ...
    return (this.menteeData.monthly(utype).subscribe((response: any) => {
      ...
      let finalData: any = [];
      response.forEach((element: any) => {
        finalData.push({ name: element.month, y: element.mark })
      });
      this.isValue = 2;
